...
public class PipelineTest {
    ...
    public void testExtraVarsHiddenString() throws Exception {
        ...
        WorkflowRun run1 = workflowJob.scheduleBuild2(0).waitForStart();
        jenkins.waitForCompletion(run1);
        assertThat(run1.getLog(), allOf(containsString("ansible-playbook playbook.yml -e ********")));
    }
    ...
    public void testExtraVarsBoolean() throws Exception {
        String pipeline = IOUtils.toString(
                PipelineTest.class.getResourceAsStream("/pipelines/extraVarsBoolean.groovy"), StandardCharsets.UTF_8);
        WorkflowJob workflowJob = jenkins.createProject(WorkflowJob.class);
        workflowJob.setDefinition(new CpsFlowDefinition(pipeline, true));
        WorkflowRun run1 = workflowJob.scheduleBuild2(0).waitForStart();
        jenkins.waitForCompletion(run1);
        assertThat(run1.getLog(), allOf(containsString("ansible-playbook playbook.yml -e ******** -e ********")));
    }

    @Test
    public void testExtraVarsNumeric() throws Exception {
        String pipeline = IOUtils.toString(
                PipelineTest.class.getResourceAsStream("/pipelines/extraVarsNumeric.groovy"), StandardCharsets.UTF_8);
        WorkflowJob workflowJob = jenkins.createProject(WorkflowJob.class);
        workflowJob.setDefinition(new CpsFlowDefinition(pipeline, true));
        WorkflowRun run1 = workflowJob.scheduleBuild2(0).waitForStart();
        jenkins.waitForCompletion(run1);
    ...
    public void testAnsiblePlaybookSshPass() throws Exception {
        ...
        jenkins.waitForCompletion(run1);
        assertThat(
                run1.getLog(), allOf(containsString("sshpass ******** ansible-playbook playbook.yml -u username -k")));
    }
    ...
    public void testVaultCredentialsFile() throws Exception {
        ...
        WorkflowRun run1 = workflowJob.scheduleBuild2(0).waitForStart();
        jenkins.waitForCompletion(run1);
        assertThat(run1.getLog(), allOf(containsString("ansible-playbook playbook.yml --vault-password-file ")));
    }
    ...
    public void testVaultCredentialsString() throws Exception {
        ...
        assertThat(run1.getLog(), allOf(containsString("ansible-playbook playbook.yml --vault-password-file ")));
        assertThat(
                run1.getLog(), not(containsString("ansible-playbook playbook.yml --vault-password-file " + tempDir)));
    }
    ...
    public void testAdhocCommand() throws Exception {
        ...
        assertThat(
                run1.getLog(),
                allOf(containsString("ansible 127.0.0.1 -i inventory -a " + "\"" + "echo something" + "\"")));
    }
    ...
    public void testVaultTmpPathString() throws Exception {
        ...
        String tempDir = getTmpPath();
        assertThat(
                run1.getLog(), allOf(containsString("ansible-playbook playbook.yml --vault-password-file " + tempDir)));
    }
}
